...
def render():
    ...
    def generate_questions(
        text, num_questions, question_type, difficulty, topic, language
    ):
        # 질문 생성 시작
        messages = []
        ...
        if not language_code:
            st.write(
                f"오류: 잘못된 언어를 선택했습니다. 다음 중에서 선택하십시오: {list(language_code_mapping.keys())}"
            )
            return
    ...
    with st.form(
        ...
        num_questions = st.slider(
            "문제 수를 선택해주세요", min_value=1, max_value=10, value=5, step=1
        )
        question_type = st.selectbox(
            "문제 형식을 선택하세요", options=["선택지 문제", "빈칸 채우기 문제", "간단 서술형 문제"], index=0
        )
        language = st.selectbox(
            "문제 언어를 선택해주세요", options=["한국어", "English", "日本語", "中文", "မြန်မာဘာသာ"]
        )
